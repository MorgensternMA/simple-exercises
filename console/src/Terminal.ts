...
type Command = 'mkdir' | 'touch' | 'cd' | 'ls' | 'lsp' | 'cat' | 'pwd' | 'close' | 'rm';
const validCommands: Command[] = ['mkdir', 'touch', 'cd', 'ls', 'lsp', 'cat', 'pwd', 'close', 'rm'];
...
export class Terminal {
  ...
  rm(name: string) {
    const target = this.current.getChild(name);
    if (target) {
      this.current.remove(target);
    } else {
